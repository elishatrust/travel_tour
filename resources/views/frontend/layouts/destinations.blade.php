
<div class="container-xxl123 py-5 our-courses" id="courses">
    <div class="container">
        <div class="row g-5 align-items-end">
            <div class="col-lg-12 col-sm-12 text-center">
                <p style="color: #f1671e;" class="text-uppercase"><span class="text-primary me-2">#</span>The Best of Tanzania</p>
                <h1 class="display-5123 h2 mb-5">Most Visited Destinations</h1>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <div class="owl-courses-item owl-carousel">
                    <div class="item">
                        <a href="{{ route('mount-meru') }}">
                            <img src="{{ asset('assets/frontend/img/MtMeru.webp') }}"  alt="#UpzoneSafaris">
                            <div class="down-content">
                                <h4 class="text-center">Mount Meru</h4>
                                <div class="info">
                                    <div class="row">
                                        <div class="col-8">
                                            <ul>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                            </ul>
                                        </div>
                                        <div class="col-4">
                                            <span>1600</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="item">
                        <a href="{{ route('arusha-national-park') }}">
                            <img src="{{ asset('assets/frontend/img/park/a4.jpg') }}"  alt="#UpzoneSafaris">
                            <div class="down-content">
                                <h4 class="text-center">Arusha National Park</h4>
                                <div class="info">
                                    <div class="row">
                                        <div class="col-8">
                                            <ul>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                            </ul>
                                        </div>
                                        <div class="col-4">
                                            <span>1200</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="item">
                        <a href="{{ route('ngorongoro-crater') }}">
                            <img src="{{ asset('assets/frontend/img/ngorongoro-crater.webp') }}"  alt="#UpzoneSafaris">
                            <div class="down-content">
                                <h4 class="text-center">Ngorongoro Crater</h4>
                                <div class="info">
                                    <div class="row">
                                        <div class="col-8">
                                            <ul>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                            </ul>
                                        </div>
                                        <div class="col-4">
                                            <span>1800</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="item">
                        <a href="{{ route('tarangire-national-park') }}">
                            <img src="{{ asset('assets/frontend/img/wildlife/kangaroo.webp') }}"  alt="#UpzoneSafaris">
                            <div class="down-content">
                                <h4 class="text-center">Tarangire National Park</h4>
                                <div class="info">
                                    <div class="row">
                                        <div class="col-8">
                                            <ul>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                            </ul>
                                        </div>
                                        <div class="col-4">
                                        <span>2700</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="item">
                        <a href="{{ route('mount-kilimanjaro') }}">
                            <img src="{{ asset('assets/frontend/img/safari/49.jpg') }}"  alt="#UpzoneSafaris">
                            <div class="down-content">
                                <h4 class="text-center">Mount Kilimanjaro</h4>
                                <div class="info">
                                    <div class="row">
                                        <div class="col-8">
                                            <ul>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                            </ul>
                                        </div>
                                        <div class="col-4">
                                            <span>1400</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="item">
                        <a href="{{ route('serengeti-national-park') }}">
                            <img src="{{ asset('assets/frontend/img/wildlife/7.webp') }}" style="width:100%;height:250px;border-radius:10px"  alt="#UpzoneSafaris">
                            <div class="down-content">
                                <h4 class="text-center">Serengeti National Park</h4>
                                <div class="info">
                                    <div class="row">
                                        <div class="col-8">
                                            <ul>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star"></i></li>
                                                <li><i class="fa fa-star" style="color: black!important"></i></li>
                                            </ul>
                                        </div>
                                        <div class="col-4">
                                            <span>2500</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
